...
var TestUI = /** @class */ (function (_super) {
    ...
    function TestUI(scene, layer) {
        var _this = _super.call(this, 'TestGUI', scene, layer) || this;
        _this.initialize();
        return _this;
    }
    ...
    TestUI.prototype.setUpControls = function () {
        var _this = this;
        var btn = this.getControl('TestButton');
        var text = this.getControl('TestText');
        var input = this.getControl('TestInput');
        var closeBtn = this.getControl('CloseBtn');
        btn.onPointerClickObservable.add(function () {
            console.log("Button Click");
            text.text = "Click ".concat(_this._scene.getFrameId());
        });
        input.onTextChangedObservable.add(function (eventData, eventState) {
            // console.log(`Input changed - data: %o  state: %o`, eventData, eventState);
            text.text = input.text;
        });
        closeBtn.onPointerClickObservable.add(function () {
            _this._root.isVisible = false;
        });
    };
    return TestUI;
